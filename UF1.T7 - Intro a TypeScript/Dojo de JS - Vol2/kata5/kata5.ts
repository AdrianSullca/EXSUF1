function minExtraChars(s: string, dictionary: string[]): number {
    let extraChars: number = 0; // Contador para caracteres sobrantes. Inicializado en 0
    let n: number = s.length; // Longitud de la cadena

    // Un bucle para recorrer cada caracter de la cadena
    for (let i = 0; i < n; i++) {
        let encontrado: boolean = false; // Variable para saber si se ha encontrado una subcadena
        
        // Intentamos formar subcadenas a partir del índice i
        for (let j = i; j < n; j++) {
            // Extraemos la subcadena de i a j
            let substring: string = s.slice(i, j + 1);
